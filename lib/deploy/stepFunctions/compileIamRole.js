...
module.exports = {
  compileIamRole() {
    ...
    this.getAllStateMachines().forEach((stateMachineName) => {
      ...
      let iamPermissions = getIamPermissions.bind(this)(taskStates, stateMachineName);
      iamPermissions = iamPermissions.concat([
        {
          action: 'states:StartExecution',
          resource: {
            'Fn::Sub': [
              `arn:\${AWS::Partition}:states:$\{AWS::Region}:$\{AWS::AccountId}:stateMachine:${stateMachineObj.name}`,
              {},
            ],
          },
        },
        {
          ...
          resource: {
            'Fn::Sub': [
              `arn:\${AWS::Partition}:states:$\{AWS::Region}:$\{AWS::AccountId}:execution/${stateMachineLogicalId}/*`,
              {},
            ],
          },
        },
